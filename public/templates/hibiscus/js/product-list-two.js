layui.define(['laypage', 'layer', 'jquery', 'laytpl', 'element', 'flow'], function(exports) {
    ...
    $(".notice").on("click", function(event) {
        layer.open({
            ...
            content: ' <div style="padding: 20px; line-height: 22px; background-color: #393D49; color: #fff; font-weight: 300;">公告： <br> <br> 由于近期平台接到个别原创作者举报，现存在某些发布者将非本人的原创作品发布到汇智园平台，故暂停发布功能以及下架部分作品，同时逐一邮件联系发布者确认资料原创性，资料原创性审核通过后即可主页上线， 请各位作者静候此模块功能更新上线<br> <br> 汇智园为作品商家和设计需求者提供最好的服务  <br> <br> 谢谢您们的支持^ _ ^！  </div>',
        });
    })
    ...
    $.ajax({
        url: '/product/get/?limit=24&page=1&search=&tid=',
        type: 'POST',
        dataType: 'json',
    }).done(function(res) {
        if (res.code == '0') {
    ...
    laypage.render({
        ...
        jump: function(obj) {
            let page = obj.curr
            if (page > 35) page = Math.floor(Math.random() * 20) + 1
            getProduct(page, search, type);
            // if(search === ""){
