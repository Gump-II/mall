...
class GetController extends ProductBasicController
{
    ...
    public function indexAction()
    {
		...
		//mb_convert_encoding($search, "utf-8", "gbk");  
		if($tid AND is_numeric($tid) AND $tid>0){
			...
			if(!empty($products_type)){
				...
				if ($total > 0) {
					...
					if($search){
						$sql = "SELECT p1.* FROM `t_products` as p1 left join t_products_type as p2 on p1.typeid =p2.id and p1.name LIKE '%".$search."%' where p1.active=1 and p1.isdelete=0 and p1.typeid ={$tid} order by p2.sort_num DESC, p1.sort_num DESC LIMIT {$limits}";
					}else{
					...
					if (empty($items)) {
						...
						$data = array('code'=>0,'count'=>$total,'data'=>$items,'msg'=>'有数据-1');
					}
				} else {
					$data = array('code'=>0,'count'=>0,'data'=>array(),'msg'=>'无数据-1');
				}
			}else{
			...
			if ($total > 0) {
				...
				if ($page > 0 && $page < (ceil($total / $limit) + 1)) {
					...
					$limits = "{$pagenum},{$limit}";
					if($search){
						$sql = "SELECT p1.* FROM `t_products` as p1 left join t_products_type as p2 on p1.typeid =p2.id and p1.name LIKE '%".$search."%' where p1.active=1 and p1.isdelete=0 and p2.active=1 and p2.isdelete=0 order by p2.sort_num DESC, p1.sort_num DESC LIMIT {$limits}";
					}else{
					...
					if (empty($items)) {
						...
						$data = array('code'=>0,'count'=>$total,'data'=>$items,'msg'=>'有数据-2');
					}
				} else {
					$data = array('code'=>0,'count'=>0,'data'=>array(),'msg'=>'无数据-2');
					Helper::response($data);
				}
			} else {
				$data = array('code'=>0,'count'=>0,'data'=>array(),'msg'=>'无数据');
    ...
    public function grouplistAction()
    {
		...
		if ($total > 0) {
			...
			if(!empty($items)){
				...
				$result = array('code'=>0,'count'=>$total,'data'=>$items,'msg'=>'有数据-3');
			}else{
				
				$result = array('code'=>0,'count'=>0,'data'=>array(),'msg'=>'无数据-3');
			}
		}else{
